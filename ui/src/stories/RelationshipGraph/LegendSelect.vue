...
import { refresh, updateGraph, updateNodes, useEchartsDrag, useEchartsGraph, useRightClickMenu } from "@/hooks/RelationshipGraph";
...
watch(graphData, () => {
    ...
    let simulation = d3.forceSimulation(d3nodes)
        .force("link", d3.forceLink(d3edges).id((node, i) => graphData.value.nodes[i].id).distance(5))//distance越大越紧凑
    ...
    chart.value.on('legendselectchanged', params => {
        ...
        simulation = d3.forceSimulation(nodes)
            .force("link", d3.forceLink(edges).id((node, i) => nodes[i].id).distance(5))//distance越大越紧凑
            .force("charge", d3.forceManyBody().strength(-1))
            //.force("charge", d3.forceManyBody().strength(1))
            .force("collide", d3.forceCollide().radius((node, i) => nodes[i].symbolSize! / 16))
            .force("x", d3.forceX())
            .force("y", d3.forceY())
            //.force('radius',d3.forceRadial(Math.min(width,height),width/2,height/2).strength(-0.1))
            //.force("center", d3.forceCenter(width / 2, height / 2))//中心力，设置之后，当画布移动后再模拟时，graph会平移
